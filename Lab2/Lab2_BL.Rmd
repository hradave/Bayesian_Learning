...
#Beta 1
hist(posterior_betas[,1], freq = F, breaks=20, main="Beta 1", col="grey", xlab="Beta values")
```

```{r}

#Beta 2
hist(posterior_betas[,2], freq = F, breaks=20, main="Beta 2", col="grey", xlab="Beta values")
```

```{r}

#Beta 3
hist(posterior_betas[,3], freq=F, breaks=20, main="Beta 3", col="grey", xlab="Beta values")
```
...
hist(x_tilda, freq=F, breaks=20, col="grey")
```


### (d)

Here we say that higher values that may not be neede, with a prior with mean=0 the problem of overfitting is solved. 
