...
larger_than_03_sim = sum(posterior_draw > 0.3) / length(posterior_draw) #0.4432
larger_than_03_true = 1 - pbeta(q = 0.3, shape1 = alpha_0 + s, shape2 = beta_0 + f) #0.4399472
```


We can compute the probability of $\theta > 0.3$ using the drawn sample and compare it to the exact number computed using \texttt{pbeta()}. The value obtained from the sample is $0.4432$ and the exact probability is $0.4399472$ - see code in Appendix. We can conclude that with this sample size both probabilities are very similar. 
...
we then plot the result in Figure \ref{fig:Gini}. From the plot we conclude that the Gini coefficient is closer to 0 than to 1, therefore the income of the people in the sample is more equal than not. 
...
In Figure \ref{fig:Kernel} we can see bla bla 
...
where $I_0(\kappa)$ is the modified Bessel function, $\mu$ is the mean direction and $\kappa$ is called the concentration parameter. 
...
$$
p(\kappa|y,\mu)= p(\kappa)\cdot p(y|\mu,\kappa)=\frac{exp[\kappa \cdot cos(y-\mu)]}{2\pi I_0(\kappa)}\cdot \lambda exp(-\lambda \kappa)
=\frac{exp[\kappa \cdot cos(y-\mu)-1]}{2\pi I_0(\kappa)}.
$$
