...
where $\hat{\beta}$ is the posterior mode and $J_y(\hat{\beta})$ is the negative Hessian at the posterior mode, which we obtain from the \texttt{optim()} function. We use a modified version of our code from Lab 2, which used Mattias Villani's code (https://github.com/mattiasvillani/BayesLearnCourse/raw/master/Code/MainOptimizeSpam.zip) as a template.
...
RWMSampler = function(logPostFunc, c, iter, initial, proposal_sigma, ...){
  ...
  for (t in 1:iter) {
    ...
    alpha = min(1,
                (exp(logPostFunc(Y, ...) - logPostFunc(X[t,], ...)))) #have to take exp(), because posterior is log
...
set.seed(1234567890)
nIter = 10000
mc = RWMSampler(LogPosterior, c = 0.65, iter = nIter, initial = Beta_init, proposal_sigma = post_sigma, y, x, mu, sigma) #parameters from 2.b
```
...
for (i in 1:dim(Beta_mcmc)[1]) {
  predictive_dist[i] = rpois(1, exp(t(as.matrix(x_pred)) %*% as.matrix(Beta_mcmc[i,]))) #sample poisson draws using Beta MCMCs
}
...
The probability that nobody bids on the coin in this new auction is 35.48%, which we get by dividing the number of predictive draws where the result is 0 by the number of all the draws. Or we can look at the histogram of the normalized predictive distribution in Figure \ref{fig:hist_pred_dist}. The low number of bids is probably because the minimum selling price for the coin was set high compared to its book value (MinBidShare = 0.5).

```{r, fig.cap="\\label{fig:hist_pred_dist} Histogram of the predicted distribution", out.width = "65%", fig.pos='!h', fig.align='center'}

barplot(values$ratio, names.arg = values$value, xlab = 'Number of bids', ylab = 'Probability density')
```
